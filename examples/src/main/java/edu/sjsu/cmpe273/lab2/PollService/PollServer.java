...
public class PollServer {
  ...
  private class PollServiceImpl implements PollServiceGrpc.PollService {
    ...
    public void createPoll(PollRequest req, StreamObserver<PollResponse> responseObserver) {
      ...
      String pollId = Integer.toString(random.nextInt(), 36).toUpperCase();
      logger.info("Creating poll for moderator id : "+req.getModeratorId());
      PollResponse reply = PollResponse.newBuilder().setId(pollId).build();
